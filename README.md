...
2. Файл <APP_CFG> перезаписывает все поля если они в нем указаны (иначе пропускает)
3. Поля связанные с длительностью выполнения математических операций, количество рабочих агента и порты из .env перезаписывают соответсвующие поля в конфигурации если существуют (даже если они указаны в yml)
